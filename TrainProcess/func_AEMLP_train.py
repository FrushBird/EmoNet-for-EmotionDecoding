...
def My_AEMLP_Train(net, num_epochs, batch_size, lr, device, dataset_train, dataset_test, random_seed=1):
...
if __name__ == '__main__':
    ...
    AE = torch.load('CNNAE_FR2Pic_16x16.pth')
    # mlp = MLP(64 * 2 * 8, 3)
    mlp = Common_CNN(3, 3)
    mlp.to('cuda')
    AECNN = PretrainAEMLP(AE, mlp)
    AECNN.to('cuda')

    data = torch.randn(10, 1, 16, 64).cuda()
    # data2 = torch.randn(2, 32, 2, 8)
    output1 = AECNN(data)
    # output2 = mlp(output1)
    print(output1.shape)
    # print(output2.shape)
